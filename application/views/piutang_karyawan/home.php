...
<div class="box">
  <div class="box-header">
    <form id="formPiKar" action="" method="post">
      <?php
      foreach ($this->M_piutangKaryawan->last_record() as $row) {
        $faktur = $row->faktur;
    ...
    <label>Nama</label>
                  <div class="input-group form-group">
                    ...
                    <select id="karyawan" name="karyawan" onchange="terimainputCombox()" class="form-control select2" aria-describedby="sizing-addon2" style="width:240px;">
                      <option value="">- Pilih Karyawan -</option>
                        <?php
                        foreach ($dataPegawai as $pegawai) {
                          ?>
                          <option value="<?php echo $pegawai->kode; ?>,<?php echo $pegawai->jabatan; ?>">
                      ...
                      <input id="kd_karyawan" type="text" class="form-control" placeholder="Kode" name="kd_karyawan" style="width:60px;" readonly>
                  </div>
                  <script>
                  function terimainputCombox(){
                         var input=document.forms['formPiKar']['karyawan'].value;
                         var outputKode = document.getElementById("kd_karyawan");
                         var outputJabatan = document.getElementById("jabatan");
                         var hasil = input.split(",");
                         outputKode.value = hasil[0];
                         outputJabatan.value = hasil[1];
                }
                  </script>
      <label>Jabatan</label>
    <div class="input-group form-group">
                    <div class="input-group date">
                      ...
                      <input type="text" class="form-control" name="jabatan" id="jabatan" style="width:250px;">
                    </div>
                    <!-- /.input group -->
                  </div>
      <label>Jumlah</label>
...
<div class="col-md-4 text-left">
  <a href="<?php echo base_url();?>PiutangKaryawan/tambahBaru" class="btn btn-primary btn-block"><i class="glyphicon glyphicon-file"></i> Tambah Baru</a>
<hr />
</div>
